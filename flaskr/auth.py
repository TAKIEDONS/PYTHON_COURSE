...
def register():
    if request.method == 'POST':
        ...
        if error is None:
            try:
               # takes a SQL query with ? placeholders for any user input, and a tuple of values 
                db.execute(
                    "INSERT INTO user (username, password) VALUES (?, ?)",
                    # securely hash the password, and that hash is stored
                    (username, generate_password_hash(password)),
                )
                db.commit()
                 # occur if the username already exists
